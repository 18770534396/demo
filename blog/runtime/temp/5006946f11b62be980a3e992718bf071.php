<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:41:"./application/admin\view\links_index.html";i:1504087778;s:42:"./application/admin\view\index_header.html";i:1514021724;s:42:"./application/admin\view\index_footer.html";i:1504421378;}*/ ?>
<!DOCTYPE html>
<html>
<head>  
...
<div class="tpt—admin">
<div class="tpt—btn">
<a href="<?php echo url('links/add'); ?>"><i class="layui-icon">&#xe61f;</i> 添加连接</a>
</div>
...
<td align="center">
<a class="layui-btn layui-btn-mini layui-btn-warm" href="<?php echo url('links/edit',array('id'=>$vo['id'])); ?>">修改</a> <a class="layui-btn layui-btn-mini layui-btn-danger del_btn" member-id="<?php echo $vo['id']; ?>" title="删除" nickname="<?php echo $vo['name']; ?>">删除</a>
</td>
</tr>
...
function changeshows(o){
  ...
  $.ajax({
	  ...
	  dataType:"json",
      data:{change:change},
	  url:"<?php echo url('links/changeshows'); ?>",
...
layui.use('form',function(){
  ...
  jq('.del_btn').click(function(){
    ...
    layer.confirm('确定删除【'+name+'】?', function(index){
      ...
      jq.post('<?php echo url("links/dels"); ?>',{'id':id},function(data){
        if(data.code == 200){
          layer.close(loading);
  ...
  form.on('submit(delete)', function(data){
    ...
    layer.confirm('确定批量删除?', function(index){
      ...
      jq.post('<?php echo url("links/delss"); ?>',param,function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();
